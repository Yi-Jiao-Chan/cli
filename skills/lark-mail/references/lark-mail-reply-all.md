...
| `--signature-id <id>` | 否 | 签名 ID。附加指定签名到回复正文与引用块之间，覆盖默认签名。省略时自动追加该发件地址的默认**回复**签名（`reply_signature_id`；账号无默认则不追加）。HTML 邮件追加 HTML 签名；纯文本邮件（`--plain-text`）追加纯文本签名（图片丢弃）。运行 `mail +signature` 查看可用签名 |
| `--no-signature` | 否 | 本次回复不追加任何签名（包括账号默认回复签名）。与 `--signature-id` 互斥 |
