...
支持通过 `--send-time` 或 `--send-after` 设置邮件的发送时间：

- `--send-time <unix_timestamp>`：使用 Unix 时间戳（秒）指定发送时间
- `--send-after <duration>`：使用相对时长指定发送时间，例如 `30m`（30分钟后）、`2h`（2小时后）

定时发送必须与 `--confirm-send` 同时使用。发送时间必须至少在当前时间的 5 分钟后。

**注意**：定时发送的邮件会在 `SCHEDULED` 文件夹中可见，可以随时取消。
...
# 定时发送（30分钟后）
lark-cli mail +send --to alice@example.com --subject '周报' \
  --body '<p>本周进展如下...</p>' --confirm-send --send-after 30m

# 定时发送（指定Unix时间戳）
lark-cli mail +send --to alice@example.com --subject '周报' \
  --body '<p>本周进展如下...</p>' --confirm-send --send-time 1744500000
```
...
| `--send-time <unix_timestamp>` | 否 | 定时发送：指定 Unix 时间戳（秒）。必须至少 5 分钟在当前时间之后。仅与 `--confirm-send` 同时使用 |
| `--send-after <duration>` | 否 | 定时发送：指定相对时长（如 30m、2h）。必须至少 5 分钟在当前时间之后。仅与 `--confirm-send` 同时使用 |
| `--dry-run` | 否 | 仅打印请求，不执行 |
