...
> **构建方式是强约束**：当 scene 指南要求"脚本生成"时，必须先写脚本（.js）并用 `node` 执行来产出 JSON
> 文件。绝对定位场景（鱼骨图、飞轮图、柱状图、折线图等）的坐标需要数学计算，直接手写 JSON 极易导致节点重叠或连线穿模。
---
...
> - **你必须立即停止操作**，并通过 `AskUserQuestion` 工具（或直接回复）向用户确认：”目标画板当前非空，继续更新将清空原有的 XX
    个节点，是否确认覆盖？”
...
你也可以将布局输出为原生 OpenAPI json 格式，再通过 lark-cli
导入飞书画板。参考 [lark-whiteboard-update.md](references/lark-whiteboard-update.md) 了解更新画板的更多细节。
...
7. **Dagre 子容器默认为不透明节点** — 外层连线无法寻址其内部子节点（引擎会自动重定向至外壳）。需穿透时声明
